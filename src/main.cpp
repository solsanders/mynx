...
#if defined(NDEBUG)
# error "Mynx cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5bd0b5e08498b24e47b483ab02a2b0fbf0358791c357dd4e63a490cc5d86fdbe");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mynx: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mynx Signed Message:\n";
...
{
    // Mynx: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 6944 * COIN;

    // Subsidy is cut in half every 1050000 blocks, which will occur approximately every 2 years
    nSubsidy >>= (nHeight / 1050000); // Mynx: 1.05M blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Mynx: 3 days
static const int64 nTargetSpacing = 1 * 60; // Mynx: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mynx: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mynx: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8539732a99dde302ea86fb93504fc95600e0e06075b8ee01bccb8d9c1198f3aa");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "13/03/24, $MYNX Has AWOKE!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 6944 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04281244c59c743b101b3830a3371f58407f13a901f50e6ff613c88109a51c42bc98763e143ef89359d0a1a3b3f1aa8adc5bd4e40405255c5ff6e481ec75fcd133") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1710342946;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084605246;

        if (fTestNet)
        {
            block.nTime    = 1710342938;
            block.nNonce   = 387074035;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb0ace84b1815d517f830b714efacb715f9db6478e477eca4c8c60f44ca0a5fe5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfl, 0xc5, 0xb3, 0xde }; // Mynx: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MynxMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MynxMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MynxMiner(CWallet *pwallet)
{
    printf("MynxMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mynx-miner");
    ...
    try { loop {
        ...
        printf("Running MynxMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MynxMiner, pwallet));
}
